...
#include <cstdio>
#include <map>
#define CK_CHECK_STATUS(STATUS, STATUS_RET, COUNTER, OK) \
   do {                                                  \
      Status st = STATUS_RET;                            \
      if (!st.IsOk()) {                                  \
         STATUS = st;                                    \
         if (--COUNTER == 0) {                           \
            OK.set_value(true);                          \
...
namespace ps {
namespace server {
//std::thread save_threat;
std::unordered_map<std::string, std::unique_ptr<Variable>> copy;
CheckpointUtils::CheckpointUtils(const VariableInfoCollection& infos)
    : infos_(infos) {}
          ...
          for (size_t si = range.begin; si < range.end; ++si) {
             ...
             for (size_t i = 0; i < to.parts.size(); i++) {
                ...
                if (part.server == id) {
                   ...
                   Status st =
                       MergeLoadVariable(to.name, vi, beg, end, &(*vars)[to.name]);
                   if (!st.IsOk()) {
                      if ((st.Code() == Status::ErrorCode::kUnknown ||
                           st.Code() == Status::ErrorCode::kNotFound) &&
                          vi.args.find("save") != vi.args.end() &&
                          vi.args["save"] == "false") {
                         LOG(INFO) << "Variable[" << to.name << "] not load.";
                         vars->erase(to.name);
                         continue;
                      }
                      LOG(WARNING) << "Variable[" << to.name
                                   << "] MergeLoadVariable failed, status["
                                   << st.Msg() << "].";
                      vars->erase(to.name);
                      return st;
                   } else {
                      LOG(INFO) << "Variable[" << to.name
                                << "] MergeLoadVariable success.";
                   }
                }
                beg = end;
             }
...
Status CheckpointUtils::saveCheckpoint(size_t id,
   ...
   for (auto&& item : vars) {
      ...
      ThreadPool::Global()->Schedule([&, name] {
         ...
         if (iter == dest_infos.end()) {
            CK_CHECK_STATUS(status,
                            Status::ArgumentError("Can't find variable[" + name +
                                                  "] in variable_infos."),
                            counter, ok);
         }
         ...
         if (part == -1) {
            CK_CHECK_STATUS(
                status,
                Status::ArgumentError("Not found variable[" + name + "] part[" +
                                      std::to_string(id) +
                                      "] in variable_infos when save variable."),
                counter, ok);
         }
         VariableStruct vs;
         CK_CHECK_STATUS(status, VariableToStruct(vars.at(name), &vs), counter,
                         ok);
         CK_CHECK_STATUS(status,
                         SaveVariable(checkpoint_path, iter->first, part, &vs),
                         counter, ok);
         if (--counter == 0) {
            ok.set_value(true);
         }
   ...
   std::future_status fstatus =
       ok.get_future().wait_for(std::chrono::minutes(timeout));
...
void CheckpointUtils::saveVar2memory(const std::unordered_map<std::string, std::unique_ptr<Variable>>& vars, std::unordered_map<std::string, std::unique_ptr<Variable>>& vars_copy) {
   for (const auto& pair : vars) {  // 遍历 unordered_map
      const std::string& key = pair.first;
      const auto& variable_ptr = pair.second;

      if (variable_ptr) {
         vars_copy[key] = variable_ptr->Clone();  // 深拷贝每个 Variable
      }
   }
}
Status CheckpointUtils::SaveVariables(
   ...
   if (save_thread->joinable()) {
      printf("Joining existing save thread.\n");
      save_thread->join();
   } else {
      printf("First time saving variables, no existing thread to join.\n");
   }
  //  if (save_threat.joinable()) {
  //     printf("Joining existing save thread.\n");
  //     save_threat.join();
  //  } else {
  //     printf("First time saving variables, no existing thread to join.\n");
  //  }
   printf("InMemoryCopy called with %zu variables.\n", vars.size());
   saveVar2memory(vars, copy);
   printf("Starting to save variables to checkpoint at %s.\n", checkpoint_path.c_str());
   *save_thread = std::thread([this, id, checkpoint_path, timeout] { saveCheckpoint(id, checkpoint_path, copy, timeout); });
   printf("SaveVariables function returning.\n");
   return Status::Ok();
}

std::string CheckpointUtils::VariableNameToFileName(const std::string& name,
                                                    size_t id) {
   std::string ret = name;
   for (auto& c : ret) {
      if (c == '/') {
...
std::string CheckpointUtils::VariableInfoToFileName(const VariableInfo& info,
                                                    size_t id) {
   auto p = info.args.find(VariableInfo::ORIGIN_FILE_PATH);
   if (p == info.args.end()) {
      LOG(ERROR) << info.name << " args not contain "
                 << VariableInfo::ORIGIN_FILE_PATH;
      return "";
   }
...
Status CheckpointUtils::MergeLoadVariable(
   ...
   for (size_t i = 0; i < info.parts.size(); i++) {
      ...
      if (part_beg < end && beg < part_end) {
         LOG(INFO) << name << ", part_beg [" << part_beg << "] part_end ["
                   << part_end << "]";
   ...
   if (info.type == VariableInfo::Type::kIndex) {
      ...
      for (const auto& lvs : variables) {
         if (lvs->beg <= beg) {
            QuickMemcpy(var.data.Raw<char>(),
                        lvs->variable.data.Raw<char>() +
                            (beg - lvs->beg) * slice_size,
                        (lvs->clip_end - lvs->clip_beg) * slice_size);
         } else {
            QuickMemcpy(var.data.Raw<char>() + (lvs->beg - beg) * slice_size,
                        lvs->variable.data.Raw<char>(),
                        (lvs->clip_end - lvs->clip_beg) * slice_size);
         }
      }
      ...
      for (auto& iter : variables[0]->variable.slots) {
         if (iter.second.joiner == Variable::SlotJoiner::kAnyOne) {
            var.slots[iter.first] = Variable::Slot{
                .tensor = std::unique_ptr<Tensor>(new Tensor(*iter.second.tensor)),
                .joiner = iter.second.joiner};
         } else {
            ...
            var.slots[iter.first] =
                Variable::Slot{.tensor = std::unique_ptr<Tensor>(new Tensor(
                                   t->Type(), s, t->GetInitializer()->Clone())),
                               .joiner = iter.second.joiner};
      ...
      result_variable->reset(
          new Variable(new Tensor(var.data), slicer.release(), name));
...
Status CheckpointUtils::LoadHashVariable(
    const std::vector<std::unique_ptr<LoadVariableStruct>>& variables,
    const std::string& name,
    const VariableInfo& info,
    size_t beg,
    size_t end,
   ...
   Variable* var =
       new Variable(new Tensor(t.Type(), data_shape, t.GetInitializer()->Clone(),
                               Tensor::TType::kSegment, false),
   ...
   for (const auto& iter : variables[0]->variable.slots) {
      if (iter.second.joiner == Variable::SlotJoiner::kAnyOne) {
         slots[iter.first] = Variable::Slot{
             .tensor = std::unique_ptr<Tensor>(new Tensor(*iter.second.tensor)),
             .joiner = iter.second.joiner};
      } else {
         ...
         slots[iter.first] =
             Variable::Slot{.tensor = std::unique_ptr<Tensor>(new Tensor(
                                tt.Type(), tt_shape, tt.GetInitializer()->Clone(),
                                Tensor::TType::kSegment, false)),
                            .joiner = iter.second.joiner};
      }
   }
   ...
   for (size_t i = 0; i < variables.size(); i++) {
      ...
      time_start = std::chrono::system_clock::now();
      size_t no_use;
      hashmap->Get((int64_t*)&key[0], value.size(), false, 1.0, &ids, nullptr,
                   &no_use, 10000000000L);
      size_t slice_size = SizeOfType(var->GetData()->Type()) *
                          var->GetData()->Shape().NumElements() /
                          var->GetData()->Shape()[0];
      for (size_t j = 0; j < ids.size(); j++) {
         ...
         for (auto& slot : slots) {
            if (slot.second.joiner == Variable::SlotJoiner::kVariableLike) {
               size_t ssize = SizeOfType(slot.second.tensor->Type()) *
                              slot.second.tensor->Shape().NumElements() /
                              slot.second.tensor->Shape()[0];
               char* target = slot.second.tensor->Raw<char>(ids[j]);
               char* source =
                   lvs->variable.slots[slot.first].tensor->Raw<char>(value[j]);
...
int64_t CheckpointUtils::CalMaxSize(
    const std::vector<std::unique_ptr<LoadVariableStruct>>& variables,
    const std::string& name,
    size_t begin,
    size_t end,
   ...
   for (size_t i = 0; i < variables.size(); i++) {
      if (variables[i]->variable.type == VariableStruct::kHashSlicer128) {
         (*keys)[i].reserve(variables[i]->variable.hash_slicer128.items.size() *
                            2);
   ...
   for (size_t i = 0; i < variables.size(); i++) {
      ...
      if (lvs->variable.type == VariableStruct::kHashSlicer128) {
         for (size_t j = 0; j < lvs->variable.hash_slicer128.items.size(); j++) {
            const HashMapItem<Hash128Key>& item =
                lvs->variable.hash_slicer128.items[j];
   ...
   LOG(INFO) << name << ", variables.size() " << variables.size() << " begin "
             << begin << " end " << end << " total " << total;
   return total;
}
...
Status CheckpointUtils::LoadVariable(const VariableInfo& info, size_t part, VariableStruct* var) {
   ...
   Status st =
       FileSystem::OpenReadStreamAny(VariableInfoToFileName(info, part), &s);
   if (!st.IsOk()) {
      LOG(ERROR) << "Open " << VariableInfoToFileName(info, part) << " failed.";
      var->initialized = false;
      return st;
   }
   return LoadVariable(info.name + " part[" + std::to_string(part) + "]",
                       s.get(), var);
}

Status CheckpointUtils::SaveVariable(const std::string& checkpoint,
                                     const std::string& var_name,
                                     size_t part,
                                     VariableStruct* var) {
   std::unique_ptr<FileSystem::WriteStream> s;
   PS_CHECK_STATUS(FileSystem::OpenWriteStreamAny(
       checkpoint + '/' + VariableNameToFileName(var_name, part), &s));
   return SaveVariable(s.get(), var);
}

Status CheckpointUtils::VariableToStruct(const std::unique_ptr<Variable>& var,
                                         VariableStruct* vs) {
   ...
   } else if (dynamic_cast<WrapperData<std::unique_ptr<HashMap>>*>(slicer) !=
              nullptr) {
      HashMap* hashmap =
          dynamic_cast<WrapperData<std::unique_ptr<HashMap>>*>(slicer)
              ->Internal()
              .get();
      if (dynamic_cast<HashMapImpl<int64_t>*>(hashmap) != nullptr) {
         vs->type = VariableStruct::kHashSlicer64;
         dynamic_cast<HashMapImpl<int64_t>*>(hashmap)->GetItems(
             &vs->hash_slicer64);
      } else if (dynamic_cast<HashMapImpl<Hash128Key>*>(hashmap) != nullptr) {
         vs->type = VariableStruct::kHashSlicer128;
         dynamic_cast<HashMapImpl<Hash128Key>*>(hashmap)->GetItems(
             &vs->hash_slicer128);
      }
   } else {
...
Status CheckpointUtils::LoadVariable(const std::string& name,
                                     FileSystem::ReadStream* s,
                                     VariableStruct* var) {
   ...
   for (size_t i = 0; i < slot_size; i++) {
      ...
      PS_CHECK_STATUS(LoadTensor(name + " slot[" + slot_name + "]", s, var->type,
                                 slot.tensor.get()));
   }
   var->initialized = true;
   return Status::Ok();
...
Status CheckpointUtils::SaveVariable(FileSystem::WriteStream* s,
                                     VariableStruct* var) {
...
Status CheckpointUtils::LoadTensor(const std::string& name,
                                   FileSystem::ReadStream* s,
                                   VariableStruct::SlicerType slicer_type,
                                   Tensor* data) {
   DataType type;
   ...
   PS_CHECK_STATUS(serializer::DeserializeAny<Initializer>(
       initializer_type, &frag, 0, &initializer, &len, mem));
   Tensor result(type, TensorShape(shape), initializer,
                 Tensor::TType::kContinuous, false);
   PS_CHECK_STATUS(s->Read(result.Raw<char>(),
                           result.Shape().NumElements() * SizeOfType(type)));
   *data = result;
   return Status::Ok();
}

Status CheckpointUtils::SaveTensor(FileSystem::WriteStream* s,
                                   const Tensor& data) {
   ...
   serializer::MemGuard mem;
   PS_CHECK_STATUS(serializer::SerializeAny<Initializer>(
       initializer, &initializer_type, &frags, mem));
   for (auto frag : frags) {
   ...
   if (data.TensorType() == Tensor::TType::kContinuous) {
      PS_CHECK_STATUS(s->Write(data.Raw<char>(),
                               tensor_shape.NumElements() * SizeOfType(type)));
      ...
      for (size_t i = 0; i < tensor_shape[0] / data.SegmentSize(); i++) {
         PS_CHECK_STATUS(
             s->Write(data.Raw<char>(i * data.SegmentSize()),
                      data.SegmentSize() * slice_size * SizeOfType(type)));
      }
   } else {
...
std::unordered_map<std::string, Variable::Slot> CheckpointUtils::CloneSlots(
    const std::unordered_map<std::string, Variable::Slot>& slots) {
   std::unordered_map<std::string, Variable::Slot> ret;
   for (auto&& item : slots) {
      ret[item.first] = Variable::Slot{
          .tensor = std::unique_ptr<Tensor>(new Tensor(*item.second.tensor)),
          .joiner = item.second.joiner};
   }
   return std::move(ret);
}
