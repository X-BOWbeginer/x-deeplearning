...
LocalServer::LocalServer(const std::string& ckpt_path)
    : udf_chain_manager_(new UdfChainManager), storage_manager_(new StorageManager), ckpt_path_(ckpt_path) {
}

Status LocalServer::Init() {
   return Status::Ok();
}
...
Status LocalServer::Save(const std::string& ckpt_version) {
   ...
   CheckpointUtils ckpt_utils(info);
   printf("hello form local server\n");
   PS_CHECK_STATUS(ckpt_utils.SaveVariables(0, real_ckpt_path, storage_manager_->Internal(),30,&save_thread));
   std::vector<std::string> checkpoints;
   {
      std::unique_ptr<FileSystem::ReadStream> s;
      Status st = FileSystem::OpenReadStreamAny(ckpt_path_ + "/checkpoints", &s);
